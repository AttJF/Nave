...
#igual o inimigo comum, ele apenas intancia uma nova bala para a fase
@export var move_speed: float = 500.0
@export var shoot_cooldown: float = 4.0 
@export var bala_scene: PackedScene
@export var bala_scene2: PackedScene
@export var powerup_scene: PackedScene
@export var powerup_scene_fi: PackedScene
...
func _death_has_come()->void:
	var b := bala_scene2.instantiate()#problema que eu estava tendo com a bala resolvido
	var parent := get_parent()
	parent.add_child(b)
	b.global_position = global_position
	...
	if b.has_method("setup"):
		b.setup(dir)
	queue_free()
	call_deferred("_lotery_deferred")
	emit_signal("died") # controle de waves 

func _lotery_deferred() -> void:#corrigir problema 
	_lotery()
