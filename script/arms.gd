...
func _shoot(direction: Vector2) -> void: # shoot padrão
	if _player == null or bala_scene == null:
		return
...
func _shoot_normal(delta: float) -> void:#modifica valores no estado normal
...
func _shoot_enraged(delta: float) -> void:#modifica no estado enraged
	if not _can_shoot(delta):
		return
	var base_dir := (_player.global_position - global_position).normalized()
	var side := base_dir.orthogonal().normalized() * 0.3# cria 2 tiros
	var dirs := [
		base_dir + side,
		base_dir - side
	]
	...
	_cool = shoot_cooldown * 0.7# diminui cooldown

func _shoot_desperated(delta: float) -> void: #cria 5 tiros
	if not _can_shoot(delta):
		return
	var base_dir := (_player.global_position - global_position).normalized()
...
func _special_enraged() -> void:
	if bala_scene == null:
		return
	var shots := 8#quantos tiros vai dar
	for i in range(shots):
		var angle := TAU * float(i) / float(shots)#tau é uma volta completa, então vai pegar a parte do angulo em radianos que vai soltar o tiro
		var dir := Vector2.RIGHT.rotated(angle)
		call_deferred("_shoot", dir) #tive que colocar isso em quase tudo pq estava dando erro
	_cool = shoot_cooldown * 1.2# setar cooldown mais rapido
...
func _got_hitr(damage:int) -> void:
	if is_invulnerable:
		return
	life -= damage
